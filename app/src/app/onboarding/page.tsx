...
export default function DappOnboardingPage() {
  ...
  const { signIn, signUp, signInWithGoogle, session, loading: authLoading } = useSupabaseAuth();
  const {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // and then back to this page after authentication
    } catch (err) {
      setSubmitState({
    ...
    <main className="min-h-screen flex flex-col pb-32 bg-black">
      ...
      <div className="flex-1 px-4 sm:px-8 pt-12 pb-6 max-w-xl mx-auto w-full space-y-8">
        ...
        <button
          ...
          disabled={isBusy}
          className="w-full px-4 py-3 rounded-lg bg-white text-gray-900 font-semibold hover:bg-gray-100 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 border border-gray-300"
        >
